...
setwd(dir = "~/Box/NLR_binding_site_prediction/analyses/ArabidopsisRENSEQ/Phylogeny/Clades/SubClades0/PairedHelper_NF/")
...
for (l in 1:length(Ent)){
  if (l>1){
    ...
    name <- colnames(d)[2]
    name <- str_replace_all(name,"X","")
    name <- str_replace_all(name,".R","-R")
    ...
    plot <- 
      ...
      ggtitle(paste(name)) +
      xlab("Position") + 
      ylab("Shannon's Entropy (Gaps Ignored)")+
      theme_classic()
    
    #folder,"/",
    pdf(paste0(folder,"/",folder,"_Entropy",".pdf"))
    print(plot)
    dev.off()
  }
}
